...
# These values are defined in speedycloud.go
...
@test "$DRIVER: create with custom disk, cpu count and memory size flags" {
  run machine create -d $DRIVER --speedycloud-cpu-count $CUSTOM_CPUCOUNT --speedycloud-disk-size $CUSTOM_DISKSIZE --speedycloud-memory $CUSTOM_MEMSIZE $NAME
  [ "$status" -eq 0  ]
}
