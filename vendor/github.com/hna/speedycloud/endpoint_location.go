...
//// deployment.
//func V2EndpointURL(catalog *tokens2.ServiceCatalog, opts EndpointOpts) (string, error) {
...
//// need to specify a Name and/or a Region depending on what's available on your OpenStack
//// deployment.
//func V3EndpointURL(catalog *tokens3.ServiceCatalog, opts EndpointOpts) (string, error) {
...
//		if (entry.Type == opts.Type) && (opts.Name == "" || entry.Name == opts.Name) {
//			for _, endpoint := range entry.Endpoints {
//				if opts.Availability != AvailabilityAdmin &&
//					opts.Availability != AvailabilityPublic &&
//					opts.Availability != AvailabilityInternal {
//					return "", fmt.Errorf("Unexpected availability in endpoint query: %s", opts.Availability)
//				}
//				if (opts.Availability == Availability(endpoint.Interface)) &&
